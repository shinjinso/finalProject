...
// 	    var pdNo = $checkbox.attr('id'); // 체크박스의 id 속성 가져오기
	...
	//지급항목 추가 화면으로 이동
	function addDed() {
		location.href = "/salary/dedInsert?ptNo=${param.ptNo}";
	}//addDed()끝
...
<body>
	...
	<h5 style="margin-left:25px;">
		...
		<a href="/salary/dedList?ptNo=${param.ptNo}" class="aTag">공제 항목</a>
	</h5>
	<hr>

	<button type="button" class="btn mb-2 btn-outline-primary"
		style="float: right; margin-bottom: 60px;" onclick="addDed()">
		+ 공제항목 추가</button>
	<br>
	<br>
	...
	<div id="MandatoryList" class="row"> <!-- 결과값 넣을 div -->
		...
		<c:forEach var="lawDed" items="${LawDedList}" varStatus="stat">
			<div class="col-md-3">
				<div class="card shadow mb-4">
					<div class="card-body text-center">
						<div class="card-text my-2 ml-1 mr-1 mb-2 row justify-content-between">
							<div>
								<strong class="card-title my-0">${lawDed.pdNm} </strong>
							</div>
							<div class="custom-control custom-switch">
								<input type="checkbox" class="custom-control-input lawPd"
									id="${lawDed.pdNo}"> 
								<label class="custom-control-label" for="${lawDed.pdNo}"></label>
							</div>
						</div>
						<div class="mt-4 mb-2 text-left" style=font-size:0.7rem;width:310px;padding-left:15px;">${lawDed.pdEpn}</div>
						<br /> 
						
					</div>
					<!-- ./card-text -->
					<div class="card-footer" style="border-top:0px;padding-top:0px;padding-bottom:25px;">
	...
	<div id="AddedList" class="row">
		...
		<c:forEach var="AddedDed" items="${AddedDedList}" varStatus="stat">
		
		<div class="col-md-3">
			<div class="card shadow mb-4">
				<div class="card-body" style="height: 200px;">
					<div class="card-text ml-1 mr-1 mb-3 row justify-content-between">
						<div>
							<strong class="card-title my-0"> ${AddedDed.pdNm} </strong>
						</div>
						<div class="row">
							...
							<div class="custom-control custom-switch mr-3">
								<input type="checkbox" class="custom-control-input"
									id="${AddedDed.pdNo}"> 
								<label class="custom-control-label" for="${AddedDed.pdNo}"></label>
							</div>
							...
							<div class="file-action mr-3">
								...
								<div class="dropdown-menu m-2">
									<a class="dropdown-item" href="/salary/dedUpdate?ptNo=${param.ptNo}&pdNo=${AddedDed.pdNo}"> 
										<i class="fe fe-edit fe-12 mr-4"></i>공제항목 수정
									</a> 
									<a class="dropdown-item" href="#" onclick="deleteDed(this)"> 
										<i class="fe fe-delete fe-12 mr-4"></i>공제항목 삭제
									</a>
								</div>
							</div>
					...
					<div class="mt-2 mb-2">
						<strong><fmt:formatNumber type="number" value="${AddedDed.pdAmt}" pattern="#,###" />원</strong> <!-- 천단위 구분 -->
					</div>
...
<script>
$.ajax({
	url:"/salary/dedListAjax",
	data :"ptNo="+"${param.ptNo}", //get방식일 때 name=value
	type:"get",
	dataType:"json",
	success:function(result){
