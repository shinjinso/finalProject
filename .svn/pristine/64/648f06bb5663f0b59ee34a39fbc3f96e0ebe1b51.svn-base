        ...
        <div id="header1">
            ...
            <div id="header2">
				...
				<div id="insaBtn" style="margin-left:32cm;margin-top:10px;">
					<button type="button" class="mainBtn" data-toggle="modal" data-target="#defaultModal"><span class="fe fe-refresh-ccw fe-16 mr-2"></span>인사발령 </button>
				</div>
                   <!-- 모달 중앙 끝 -->
            </div>
	            <div>
...
<script type="text/javascript">
	function fill(){
		...
		$("select[name=team]").val("3");
		$("select[name=promotion]").val("전환배치");
		$("input[name=date]").val("2023-10-17");
		$("textarea[name=rsn]").val("인사 오류");
 		$("textarea[name=note]").val("인사 오류로 인한 전환배치");
	}
